...
(function ($) {
    ...
    $.supersized = function (options) {
        ...
        var settings = {
            ...
            //Flickr
            source: 1,
            //1-Set, 2-User, 3-Group
            set: '72157626955422906',
            //Flickr set ID (found in URL)
            user: '19327998@N00',
            //Flickr user ID (http://idgettr.com/)
            group: '###',
            //Flickr group ID (http://idgettr.com/)
            total_slides: 250,
...
/**
    		...
    		**/
            api_key: '7a3769f28a1d3efb7db26a5af9ebc198' //Flickr API Key
        };
        ...
        case 1:
            //From a Set
            var flickrURL = 'https://api.flickr.com/services/rest/?&method=flickr.photosets.getPhotos&api_key=' + options.api_key + '&photoset_id=' + options.set + '&per_page=' + options.total_slides + '&format=json&jsoncallback=?';
            break;
        case 2:
            //From a User
            var flickrURL = 'https://api.flickr.com/services/rest/?format=json&method=flickr.photos.search&api_key=' + options.api_key + '&user_id=' + options.user + '&per_page=' + options.total_slides + '&jsoncallback=?';
            break;
        case 3:
            //From a Group
            var flickrURL = 'https://api.flickr.com/services/rest/?format=json&method=flickr.photos.search&api_key=' + options.api_key + '&group_id=' + options.group + '&per_page=' + options.total_slides + '&jsoncallback=?';
            break;
        ...
        $.ajax({ //request to Flickr
            ...
            success: function (data) {
                ...
                if (options.slides.length > 1) {
                    ...
                    $("<img/>").attr("src", options.slides[loadPrev].image).appendTo(element).wrap('<a ' + linkTarget + '></a>');
                }

                //Set current image
                imageLink = (options.slides[currentSlide].url) ? "href='" + options.slides[currentSlide].url + "'" : "";
                $("<img/>").attr("src", options.slides[currentSlide].image).appendTo(element).wrap('<a class="activeslide" ' + linkTarget + '></a>');
                ...
                if (options.slides.length > 1) {
                    ...
                    $("<img/>").attr("src", options.slides[loadNext].image).appendTo(element).wrap('<a ' + linkTarget + '></a>');
                }
        ...
        if (options.keyboard_nav) {
            ...
            $('.activeslide', document.body).live('click', function (event) {
        ...
        function nextslide() {
            ...
            $("<img/>").attr("src", options.slides[loadSlide].image).appendTo(element).wrap("<a " + linkTarget + "></a>"); //Append new image
        ...
        function prevslide() {
            ...
            imageLink = (options.slides[loadSlide].url) ? "href='" + options.slides[loadSlide].url + "'" : ""; //If link exists, build it
            $("<img/>").attr("src", options.slides[loadSlide].image).prependTo(element).wrap("<a " + linkTarget + "></a>"); //Append new image
            //Update thumbnails (if enabled)
            if (options.thumbnail_navigation == 1) {
